...
def channel_crawl(channel_URL,start_page,end_page):
    ...
    while page<end_page:
        ...
        URL=channel_URL+"?target=comment&keyword=Name&p=1"#hardcod
        first_flag=False
        ...
        for article in articles:
            ...
            article_id=int(str(str(article.get_attribute('href')).split('/')[5]).split('?')[0])
            # str_article_date=str(article_date_driver[0].get_attribute('datetime')).replace("T", " ").replace(".000Z", "") #댓글 검색으로는 값이 안나옴
            # article_date=datetime.datetime.strptime(str_article_date, '%Y-%m-%d %H:%M:%S')
            article_date=0#댓글 검색으로는 값이 안나옴
        ...
        for article_id, article_date in zip(article_id_list,article_date_list):    
            ...
            print(str(datetime.datetime.now().strftime('[%H:%M:%S] '))+"본글 아이디:"+str(article_id) + " 시간:모름")
